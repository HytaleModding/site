...
Todo mundo sabe o que é um link. No entanto, existe uma diferença entre vincular a um recurso externo e vincular a um recurso interno.
...
Você não pode usar links comuns aqui. Digamos que o seu link para outro guia seja `https://hytalemodding.dev/en/docs/quick-start`.
Você não pode escrever dessa forma porque não está se referindo apenas à página, mas também ao idioma, o que não é correto.
Em vez disso, digite `./quick-start`, que é o correto e vincula à página no idioma do usuário.

**Acessando arquivos**

- **Pasta atual**: `./`
- **Pasta acima**: `../` tantas vezes quanto necessário, de acordo com o nível desta pasta na hierarquia.

## Ícones

Se você quiser adicionar um ícone para a sua página, deve usar o [Lucide](https://lucide.dev/).
...
Estes são tipos de callouts; eles são opcionais e mostram o quão complexa a sua página pode ser.
Existem 3 níveis principais de complexidade.

<Callout type="lvl_beginner">
    Use `lvl_beginner`. Esta página contém informações para aqueles que estão começando com a criação de mods.
</Callout>

<Callout type="lvl_intermediate">
    Use `lvl_intermediate`. Esta página contém informações para aqueles que desejam aprofundar seus conhecimentos.
</Callout>

<Callout type="lvl_advanced">
   Use `lvl_advanced`. Esta página contém informações para aqueles que já sabem criar mods de qualidade e inclui conteúdo adicional.
</Callout>

O uso deles é o mesmo de um callout, mas recomendamos não alterar o título. Dentro dele, você escreve o porquê da sua página ser classificada com este nível e o que o usuário deve saber.
