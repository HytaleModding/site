---
title: Hologramas de título
description: Aprenda a criar hologramas de título.
authors:
  - name: Bird
...
1. Abra e equipe a ferramenta **Seletora de Entidades**.
...
public class TitleHologramCommand extends CommandBase {
    ...
    public TitleHologramCommand() {
        super("TitleHologram", "Cria um holograma de título.");
    }
...
| Valor             | Finalidade                                    |
| ----------------- | --------------------------------------------- |
| `playerUUID`      | O ID único do remetente do comando            |
| `playerRef`       | Uma referência do jogador no lado do servidor |
| `world`           | O mundo em que o jogador está atualmente      |
| `playerTransform` | A posição e a rotação do jogador              |
...
holder.addComponent(
    ...
    new Nameplate("Testando hologramas")
);
```

| Componente  | Finalidade                               |
| ----------- | ---------------------------------------- |
| `NetworkId` | Permite que a entidade seja sincronizada |
| `Nameplate` | O texto mostrado pelo holograma          |

---

### 7. Gerando a entidade
...
Isso insere a entidade do holograma no mundo, tornando-a ativa e visível.

---

![tile-hologram-5](/assets/guides/tile-hologram-5.png)

_Agradecimentos a Al3xWarrior e Quito no Discord por descobrirem inicialmente esse recurso._
_E agradecimentos a Elie pelo trecho de código inicial._
