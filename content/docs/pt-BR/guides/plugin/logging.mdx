...
Existe um método `.forEnclosingClass()` que você pode usar para obter automaticamente o logger do arquivo em que você está trabalhando, sem precisar alterá-lo toda vez.
...
Modify a plugins logging level by using the `/log` command

```
/log --help
/log <PLUGIN> --level <LEVEL>
/log MyCustomPlugin --level fine
```
