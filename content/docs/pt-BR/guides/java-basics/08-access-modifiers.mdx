...
A static field represents a data member owned by the class type rather than the object. Os campos estáticos são armazenados em um local de memória específico, compartilhado entre todas as instâncias de objetos criadas.
...
Pegaremos o mesmo exemplo da classe `Data` e adicionaremos este construtor:
...
A partir da classe `Data`, considere o método de instância `foo`e o método estático `bar`.
...
Data.bar(); // Método estático: acessível sem um objeto
```
