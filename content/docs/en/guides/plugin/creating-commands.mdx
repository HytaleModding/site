...
`AbstractAsyncCommand` runs on its own background thread. This means by default it is not tied to any world instance. Because of this it can not edit any Stores or Refs without getting the desired world first as the world needs to ececute the command in its thread. To access this data any of the other command types would be prefered and are likely what you will be using instead.

The main purpose for a command like this would be for something that can happen everywhere and is not tied to a world. For example a `serverRules` command to tell the user thats running the command what are the servers rules are nomatter what world they are in. 
...
Remember to add the command to the command registry in your main plugin class!
```java
