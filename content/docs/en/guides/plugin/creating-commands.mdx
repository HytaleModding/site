...
- Arguments
    - [Argument Types](#command-argument-types)
...
*NOTE:* Remember to add the command to the command registry in your main plugin class to be able to use the command.
```java
...
Argument validators are a way to check the command before the command gets executed. They are added with the `addValidator` method on an argument. They also provide a nice output to the user when an input value is invalid. 
...
...
    ...
    public ServerRulesCommand() {
        ...
        // Use or block if it requires one of a list of permissions roles
        requirePermission (
            PermissionRules.or (
